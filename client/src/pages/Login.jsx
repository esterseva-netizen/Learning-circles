...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrors({ general: err.response?.data?.message || 'אימייל או סיסמה שגויים' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ minHeight: '100vh', fontFamily: 'Heebo, sans-serif', direction: 'rtl' }}>
      <style>{`
        ...
        .feature-card:nth-child(1) { animation: float 3s ease-in-out infinite; }
        .feature-card:nth-child(2) { animation: float 3s ease-in-out infinite; animation-delay: 0.5s; }
        .feature-card:nth-child(3) { animation: float 3s ease-in-out infinite; animation-delay: 1s; }
        .feature-card:nth-child(4) { animation: float 3s ease-in-out infinite; animation-delay: 1.5s; }
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', background: '#F7F9FC' }}>
          <div className="login-card" style={{
            ...
            <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
              <h2 style={{ margin: 0, color: '#1E293B', fontSize: '1.8rem', fontWeight: '700' }}>ברוכים הבאים 👋</h2>
              <p style={{ margin: '8px 0 0', color: '#64748B', fontSize: '15px' }}>התחברו כדי להמשיך ללמוד עם הקהילה</p>
            </div>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={{ textAlign: 'left', marginBottom: '1.5rem' }}>
                <a href="#" style={{ color: '#5B5FEF', fontSize: '13px', fontWeight: '500', textDecoration: 'none' }}>שכחת סיסמה?</a>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', margin: '1.5rem 0' }}>
                ...
                <span style={{ color: '#64748B', fontSize: '13px' }}>או</span>
                <div style={{ flex: 1, height: '1px', background: '#E2E8F0' }} />
              </div>
              ...
              <button type="button" className="google-btn" style={{
                ...
                <span style={{ fontSize: '20px' }}>🌐</span> המשך עם Google
              </button>

              <p style={{ textAlign: 'center', marginTop: '1.5rem', fontSize: '14px', color: '#64748B' }}>
                אין לך חשבון?{' '}
                <Link to="/register" style={{ color: '#5B5FEF', fontWeight: '700', textDecoration: 'none' }}>הירשם עכשיו</Link>
              </p>
            </form>
          </div>
        </div>
